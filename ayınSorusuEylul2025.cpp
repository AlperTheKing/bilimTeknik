...
struct Runner {
    mt19937_64 rng;
    int lookahead = 1;     // 0: off, 1: on, 2: 2-ply lookahead
        ...
        for(int step=0; step<64; ++step){
            ...
            if(doRandom){
                ...
                for(const Cand& x : pool){
                    ...
                    int score1 = count_odds_after(occ2); // 1-ply: how many odds next
                    int sc = score1;
                    if(lookahead >= 2){
                        // 2-ply: assume next step also maximizes odd count
                        int best2 = 0;
                        for(int r2=0;r2<8;++r2){
                            for(int c2=0;c2<8;++c2){
                                int t = idx(r2,c2);
                                if( ((occ2>>t)&1ULL) ) continue;
                                int sc2 = count_odds_after(occ2 | (1ULL<<t));
